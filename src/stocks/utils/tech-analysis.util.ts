...
export function calculateTechnicalSignals(prices: number[]): TechnicalSignals {
    ...
    let summary = '';
    if (rsi < 30) summary = 'Stock is significantly oversold, suggesting a potential rebound.';
    else if (rsi > 70) summary = 'Stock is in overbought territory, prone to a correction.';
    else if (trend === 'BULLISH') summary = 'Strong bullish trend confirmed by moving average alignment.';
    else if (trend === 'BEARISH') summary = 'Bearish momentum persists as price stays below short-term averages.';
    else summary = 'Consolidation phase with neutral momentum indicators.';

    return { rsi, trend, signal, score, summary };
}

export function generateSyntheticRationale(signals: TechnicalSignals, symbol: string): string {
    const { rsi, trend, signal, summary } = signals;
    const ticker = symbol.split('.')[0];

    const intro = `Technically analyzed data for **${ticker}** indicates a **${signal}** bias. `;
    const rsiPart = rsi > 70 ? `With an RSI of ${rsi.toFixed(1)}, the stock is currently in **overbought territory**, suggesting caution. ` :
        rsi < 30 ? `With an RSI of ${rsi.toFixed(1)}, the stock is significantly **oversold**, potentially nearing a reversal. ` :
            `The RSI is stable at ${rsi.toFixed(1)}, showing balanced momentum. `;

    const trendPart = trend === 'BULLISH' ? `Price action remains above major moving averages, confirming a **strong bullish trend**. ` :
        trend === 'BEARISH' ? `The stock continues to trade below key resistance levels, maintaining a **bearish trajectory**. ` :
