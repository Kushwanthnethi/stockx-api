...
export class VerdictsService implements OnModuleInit {
    ...
    private readonly NIFTY_50_SYMBOLS = [
        'RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS', 'ICICIBANK.NS', 'INFY.NS', 'BHARTIARTL.NS', 'ITC.NS', 'SBIN.NS', 'LICI.NS',
        'LT.NS', 'HINDUNILVR.NS', 'BAJFINANCE.NS', 'HCLTECH.NS', 'KOTAKBANK.NS', 'M&M.NS', 'ADANIENT.NS', 'SUNPHARMA.NS',
        'MARUTI.NS', 'TITAN.NS', 'AXISBANK.NS', 'ULTRACEMCO.NS', 'ASIANPAINT.NS', 'NTPC.NS', 'ADANIPORTS.NS',
        'POWERGRID.NS', 'ONGC.NS', 'WIPRO.NS', 'COALINDIA.NS', 'NESTLEIND.NS', 'BAJAJFINSV.NS', 'JSWSTEEL.NS', 'ADANIGREEN.NS',
        'TATASTEEL.NS', 'GRASIM.NS', 'TECHM.NS', 'HDFCLIFE.NS', 'INDUSINDBK.NS', 'LTIM.NS', 'HINDALCO.NS', 'DIVISLAB.NS',
        'ADANIPOWER.NS', 'CIPLA.NS', 'EICHERMOT.NS', 'BPCL.NS', 'BRITANNIA.NS', 'APOLLOHOSP.NS', 'HAL.NS', 'TATACONSUM.NS'
    ];
    ...
    private async markNifty50Stocks() {
        try {
            this.logger.log(`Marking ${this.NIFTY_50_SYMBOLS.length} stocks as Nifty 50...`);
    ...
    async getNifty50Verdicts() {
        ...
        const stocksWithInstitutional = await Promise.all(stocks.map(async (s: any) => {
            const tech = await this.stocksService.getTechnicalAnalysis(s.symbol);
            const details = await this.stocksService.getEarningsDetails(s.symbol).catch(() => null);

            // Calculate Institutional Score
            const instVerdict = this.calculateInstitutionalVerdict(tech, details, s.changePercent);
        ...
        const validStocks = stocksWithInstitutional.filter(s =>
    ...
    async runSmartRefreshCycle(stocks: any[]) {
        ...
        for (const stock of stocks) {
            ...
            if (!latestVerdict) {
                ...
                const hoursSinceLast = (Date.now() - new Date(latestVerdict.generatedAt).getTime()) / (1000 * 60 * 60);
        ...
        if (toRefresh.length > 0) {
            ...
            this.logger.log(`Added ${toRefresh.length} stocks to refresh queue. Total: ${this.refreshQueue.length}`);
            this.startWorker();
        }
    }
    ...
    private async processQueueWorker() {
        while (this.refreshQueue.length > 0) {
            ...
            if (this.aiConfig.isOverloaded) {
                this.logger.warn('AI WORKER: Service currently overloaded. Sleeping for 1 minute.');
                await this.aiConfig.delay(60000);
                continue;
            }

            this.logger.log(`AI WORKER: Starting batch. Progress: ${this.refreshQueue.length} remaining.`);
            ...
            for (const symbol of batch) {
                ...
                if (this.aiConfig.isAllExhausted) {
                    this.logger.warn('AI WORKER: All keys exhausted. pausing and re-queuing current batch.');
            ...
            this.logger.log(`AI WORKER: Batch finished (${successCount}/${batch.length} success).`);
    ...
    async generateSingleVerdict(symbol: string, retryCount = 0): Promise<boolean> {
        try {
            this.logger.log(`Generating Verdict for ${symbol}...${retryCount > 0 ? ' (Retry ' + retryCount + ')' : ''}`);
            ...
            const earnings = await this.stocksService.getEarningsDetails(symbol).catch(() => null);
            ...
            if (!model) {
                this.logger.error(`No AI model available for ${symbol}. All keys might be exhausted.`);
                return false;
            }
            ...
            this.logger.log(`Gemini API responded for ${symbol} in ${endTime - startTime}ms.`);
            ...
            await (this.prisma as any).stockVerdict.create({
                data: {
                    ...
                    validUntil: new Date(Date.now() + (data.valid_duration_days || 7) * 24 * 60 * 60 * 1000)
                }
            });
            ...
            if (e.message?.includes('429') || e.message?.includes('quota')) {
                ...
                if (retryCount < 1 && !this.aiConfig.isAllExhausted) {
                    this.logger.log(`Quota hit during ${symbol}. Key rotated. Retrying in 5s...`);
                ...
                this.logger.error(`Model Overloaded for ${symbol}. Triggering global cooldown.`);
    ...
    private calculateInstitutionalVerdict(tech: any, details: any, change24h: number) {
        ...
        if (details?.recommendationMean) {
            ...
            if (mean <= 1.5) score += 4; // Strong Buy
            else if (mean <= 2.5) score += 2; // Buy
            else if (mean >= 4.0) score -= 4; // Sell
        ...
        // Final Mapping
        let verdict = 'HOLD';
        let confidence = 50 + (Math.abs(score) * 4);
