...
// Helper to perform manual upsert to avoid TS errors if Unique constraints aren't fully recognized by generated types
async function manualUpsertInvestor(prisma: PrismaClient, data: any) {
...
export async function seedInvestors(prisma: PrismaClient) {
    ...
    const jhunjhunwala = await manualUpsertInvestor(prisma, {
    ...
    // 2. Vijay Kedia
    const kedia = await manualUpsertInvestor(prisma, {
        name: 'Vijay Kedia',
    ...
    // 3. Dolly Khanna
    const dolly = await manualUpsertInvestor(prisma, {
        name: 'Dolly Khanna',
        ...
        imageUrl: 'https://trendlyne.com/ui/assets/images/investor_avatar/Dolly-Khanna.jpg',
        strategy: 'Small Cap / Cyclicals',
        netWorth: '₹800 Cr+'
    });

    // 4. Ashish Kacholia
    const kacholia = await manualUpsertInvestor(prisma, {
    ...
    const damani = await manualUpsertInvestor(prisma, {
